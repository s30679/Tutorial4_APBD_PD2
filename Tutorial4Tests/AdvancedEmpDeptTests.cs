...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Join(emps,ea => ea.Mgr,eb => eb.EmpNo,(ea, eb) => new {Employee=ea.EName,Manager=eb.EName});
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(depts, e => e.DeptNo, d => d.DeptNo, (e, d) => new {e, d}) 
            .SelectMany(ed => grades.Where(s => ed.e.Sal>=s.Losal && ed.e.Sal<=s.Hisal).
                    Select(s => new {ed.e.EName, ed.d.DName, s.Grade}), (ed, s) => new {ed.e.EName, ed.d.DName, s.Grade});
